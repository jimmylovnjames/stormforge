...
| `blackswan <program>` | Rarity-weighted multi-step campaign scenarios |
...
4. **Collect:** `findings <program>` (JSON), `report <program>` (Markdown),
   `chains <program>` (composites), and `blackswan <program>` (campaign scenarios).
...
7. **Black Swan (special):** `GET {BASE}/api/black-swan/<program>` (or `?format=md`) builds
   rarity-weighted, multi-step exploit campaigns from co-occurring findings.
   Add `?blackSwan=1` to `/api/report` or `/api/triage` to include synthetic
   `black-swan-*` findings in filing workflows.
8. **OAST (if enabled):** after an active scan, `POST {BASE}/api/oast/poll` to correlate
...
| GET | `/api/report/:program` | – | Markdown disclosure (`?submitReady=1`, `?minSeverity=`, `?blackSwan=1`) |
| GET | `/api/triage/:program` | – | Ranked submit queue (`?format=md`, `?ready=1`, `?limit=N`, `?blackSwan=1`) |
