...
<main>
  <section class="panel">
    ...
    <label>Session cookie <span class="muted">(optional — enables auth dual-pass / IDOR)</span></label>
    <input id="sessionCookie" type="password" autocomplete="off" placeholder="session=…; other=…" />
    <label>Authorization header <span class="muted">(optional)</span></label>
    <input id="sessionAuth" type="password" autocomplete="off" placeholder="Bearer eyJ…" />
    <div class="check">
...
function buildRequest() {
  ...
  const authorization = $('sessionAuth').value.trim();
  ...
  if (cookie || authorization) body.session = session;
  return body;
}
