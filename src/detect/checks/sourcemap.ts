...
function isSourceMapJson(body: string): boolean {
  if (!/"version"\s*:\s*3/.test(body)) return false;
  return /"sources"\s*:\s*\[/.test(body) || /"mappings"\s*:/.test(body);
}

function hasSourcesContent(body: string): boolean {
  return /"sourcesContent"\s*:\s*\[/.test(body) && !/"sourcesContent"\s*:\s*\[\s*\]/.test(body);
}

function extractSourceMappingUrls(body: string, baseUrl: string): string[] {
  const out: string[] = [];
