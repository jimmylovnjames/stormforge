...
export const authAccessCheck: Check = {
  ...
  run(probe: ProbeResult): Finding[] {
    ...
    const severity: Finding['severity'] =
      hasAdmin || pathClass.kind === 'admin-bypass' ? 'critical' : 'high';
...
export function classifyPath(url: string): PathClass | null {
  ...
  if (
    ...
    /\/api\/v\d+\/(?:users?|accounts?|profiles?|orders?)\/(?:\d+|me)(?:\/|$)/.test(pathname)
  ) {
    return { kind: 'idor', label: pathname };
  }

  if (
    /\/(?:api\/)?(?:v\d+\/)?(?:me|profile|settings|account|dashboard)(?:\/|$)/.test(pathname) ||
    /\/api\/v\d+\/user(?:\/|$)/.test(pathname)
  ) {
