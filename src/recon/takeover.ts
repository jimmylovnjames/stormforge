...
export const TAKEOVER_FINGERPRINTS: Array<{
  ...
  { service: 'Heroku', cname: /\.herokuapp\.com$/i, body: /no such app|No such app/i, severity: 'high' },
  {
    ...
    cname: /\.s3-website[.-]/i,
    body: /NoSuchBucket|The specified bucket does not exist/i,
    severity: 'critical',
  },
...
/** Match HTTP body fingerprints for unclaimed SaaS pages. */
export function matchTakeoverBody(body: string): { service: string; severity: 'high' | 'critical' } | null {
  if (!body || body.length < 12) return null;
  for (const fp of TAKEOVER_FINGERPRINTS) {
    if (fp.body && fp.body.test(body)) {
      return { service: fp.service, severity: fp.severity };
    }
  }
  return null;
}
...
export function parseDohResponse(
  host: string,
  json: {
    Status?: number;
